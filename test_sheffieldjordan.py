...
def f04(string):
    ...
    x = int(len(string))-1
    while x > -1:
        reversed_text.append(letters_list[x])
        x -= 1
    else:
...
def f06(string1, string2):
    bigger = max(string2, string1, key=len)
    if bigger == string1:
        extra = len(string1)-len(string2)
        print(bigger, "is longer than", string2, "by {} chars".format(extra))
        print(string2, "has only {}% the number of chars of".format(round(float(len(string2)/len(string1)*100.00))), string1)        
    if bigger == string2:
        extra = len(string2)-len(string1)
        print(bigger, "is longer than", string1, "by {} chars".format(extra))
        print(string1, "has only {}% the number of chars of".format(round(float(len(string1)/len(string2)*100.00))), string1)                
...
#   [1.0, 1.3, 2.443]
def f12():
    with open("log_file.txt","w") as file_handle:
        while True:
            try:
                while user_input = float(input("Would you please provide input? "))
                if type(user_input) != type(float):
                     log_file.txt.write(user_input, "\n")
            except ValueError:
